...
\begin{tabular}{@{}lccc@{}}
\toprule
\textbf{Method} & \textbf{Fold 1} & \textbf{Fold 2} & \textbf{CV Mean $\pm$ Std} \\
\midrule
1-NN Baseline & 0.9175 & 0.9115 & 0.9065 $\pm$ 0.0116 \\
3-NN & 0.9115 & 0.8966 & 0.8995 $\pm$ 0.0089 \\
5-NN & 0.9130 & 0.8906 & 0.8990 $\pm$ 0.0100 \\
LDA & 0.7271 & 0.7136 & 0.7220 $\pm$ 0.0060 \\
QDA + PCA50 & 0.9265 & 0.9205 & 0.9220 $\pm$ 0.0021 \\
SVM-Linear & 0.8441 & 0.8456 & 0.8460 $\pm$ 0.0018 \\
SVM-RBF & 0.9355 & 0.9265 & 0.9265 $\pm$ 0.0074 \\
SVM-RBF + PCA50 & 0.9520 & 0.9430 & 0.9425 $\pm$ 0.0080 \\
Logistic Regression & 0.8396 & 0.8621 & 0.8550 $\pm$ 0.0109 \\
\textbf{SVM-RBF + PCA35 + Aug} & \textbf{0.9670} & \textbf{0.9565} & \textbf{0.9585 $\pm$ 0.0063} \\
SVM-RBF + PCA50 + Aug & 0.9670 & 0.9610 & 0.9585 $\pm$ 0.0082 \\
\bottomrule
\end{tabular}
\end{table}
...
We analyze the confusion matrices of the selected model (SVM-RBF + PCA35 + Aug) across both trials. The top confused digit pairs are:
\begin{itemize}[noitemsep]
...
Per-digit accuracy for the selected model (SVM-RBF + PCA35 + Aug) is high across all classes, with the lowest accuracy on digit 3 (93.00\%) and digit 9 (92.75\%). Digit 1 is the easiest (98.75\%), followed by digit 0 (97.75\%).
...
\textbf{Effect of data augmentation.} Mild geometric augmentation (rotation $\pm 5^\circ$, translation $\pm 1$px, scale $0.95$--$1.05$, $\times 3$ copies per sample) is the single most impactful improvement in this study. It raises MNIST test accuracy from 94.90\% to 96.37\% and challenge accuracy from 71.67\% to 83.33\%. The augmentation introduces controlled diversity into the training set, helping the model learn features invariant to small pose variations. Importantly, stronger augmentation with noise and brightness changes actually degrades performance, confirming that excessive distortion destroys useful signal. The optimal augmentation is light enough to preserve digit structure while providing regularization benefits.
...
\begin{enumerate}
    ...
    \item \textbf{Data augmentation is the single most impactful improvement}: it raises challenge accuracy by 11.67 percentage points over the non-augmented model, effectively closing the generalization gap between student-written and professor-written digits. Cross-validation confirms mild augmentation outperforms both no augmentation and strong augmentation.
    \item \textbf{Hyperparameter sensitivity is low in the optimal region.} GridSearchCV over PCA components [20, 25, 30, 35, 40] and SVM $C$ [3, 4, 5, 6, 8] shows a broad plateau around PCA 35--40 and $C=4$, confirming robust parameter choices.
