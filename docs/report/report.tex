...
\begin{tabular}{@{}llll@{}}
\toprule
\textbf{Clf.} & \textbf{Norm.} & \textbf{PCA} & \textbf{Aug.} \\
\midrule
1-NN/3-NN/5-NN & None & -- & -- \\
LDA & None & -- & -- \\
QDA + PCA50 & None & 50 & -- \\
SVM-Linear & Scale $[0,1]$ & -- & -- \\
SVM-RBF & Scale $[0,1]$ & -- & -- \\
SVM-RBF + PCA50 & Scale $[0,1]$ & 50 & -- \\
Logistic Reg. & Standard & -- & -- \\
SVM-RBF + PCA35 + Aug & Scale $[0,1]$ & 35 & Mild$\times$3 \\
SVM-RBF + PCA50 + Aug & Scale $[0,1]$ & 50 & Mild$\times$3 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table*}[t]
\centering
\caption{Final test accuracy of the selected model on both predefined trials.}
...
Challenge dataset results are reported in Appendix~\ref{app:challenge}.

\subsection{Parameter Sensitivity Analysis}

To understand how hyperparameters affect performance, we conduct a controlled sensitivity study on data augmentation intensity using 3-fold stratified cross-validation on the training data only. A complementary grid search over PCA components and SVM regularization parameter $C$ is reported in Appendix~\ref{app:gridsearch}.
...
To illustrate \emph{why} augmentation is necessary, Figure~\ref{fig:augmentation-necessity} shows test samples that are misclassified without augmentation but correctly classified with mild augmentation. Across both trials, 65 samples (37.0\% and 39.3\% of all errors in Trial~1 and Trial~2, respectively) are fixed solely by introducing small geometric variations during training. Many of these are digits with slight rotations, translations, or stroke variations that the non-augmented model has not seen during training.

\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{figures/augmentation_necessity_combined.png}
\caption{Samples fixed by data augmentation (SVM-RBF + PCA35). Each panel shows a test image with its true label, the prediction without augmentation, and the prediction with mild geometric augmentation ($\times$3). Green titles indicate the augmented model is correct. These samples represent ambiguous or slightly transformed digits that the non-augmented model fails to recognize.
}
\label{fig:augmentation-necessity}
\end{figure*}
...
\begin{enumerate}
    ...
    \item \textbf{Hyperparameter sensitivity is low in the explored region.} GridSearchCV over PCA components [20, 25, 30, 35, 40] and SVM $C$ [3, 4, 5, 6, 8] (Appendix~\ref{app:gridsearch}) shows a broad plateau with the peak at PCA~30 and $C=5$ (96.25\% CV). The selected configuration (PCA~35, $C=4$) is near-optimal, confirming robust parameter choices.
...
\section{Challenge Dataset Evaluation}
\label{app:challenge}
...
\section{SVM Hyperparameter and PCA Component Selection}
